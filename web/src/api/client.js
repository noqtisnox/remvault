...
export const api = {
  ...
  campaigns: {
    ...
    removeMember:  (id, userId)   => request('DELETE',`/campaigns/${id}/members/${userId}`),
    getSessions:   (id)           => request('GET',   `/campaigns/${id}/sessions`),
    createSession: (id, data)     => request('POST',  `/campaigns/${id}/sessions`, data),
    updateStatus:  (cid, sid, d)  => request('PATCH', `/campaigns/${cid}/sessions/${sid}/status`, d),
    updateNotes:   (cid, sid, d)  => request('PATCH', `/campaigns/${cid}/sessions/${sid}/notes`, d),
  }
}